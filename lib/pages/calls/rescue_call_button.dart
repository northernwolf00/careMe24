  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
            ...
            children: [
              GestureDetector(
                onTap: ()async{
                  ...
                  if (on){
                    RequestStatusModel resopnse = await AppBloc.requestCubit.createRequest(widget.text, 'mch', false, institutionModel?.id ?? '');
                    if (resopnse.isSuccess) {
                      Navigator.push(context, MaterialPageRoute(builder: (context) => MainCallPage(text: 'Вызов MЧС', requestId: resopnse.requestId,show: isNotifContact, type: 'mch',)));
                    }else{
                      ElegantNotification.error(
                        description: Text('Неудалось вызвать MЧС')
                      ).show(context);
                      setState(() {
                        on = false;
                      });
                ...
                child: SvgPicture.asset(on ?'assets/images/r_on.svg' : 'assets/images/r_off.svg')),
              Text('Вызвать МЧС',style: TextStyle(color: Color.fromRGBO(219, 19, 91, 1), fontSize: 18, fontWeight: FontWeight.w600)),
            ],
          ),
        ),
        SizedBox(height: 20),
        ...
        ? GestureDetector(
          onTap: ()async{
            Navigator.push(context, MaterialPageRoute(builder: (context) => SelectInstituts(type: 'mch')))
            .then((institution){
              setState(() {
