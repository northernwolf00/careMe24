  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
            ...
            children: [
              GestureDetector(
                onTap: ()async{
                  ...
                  if (on){
                    RequestStatusModel resopnse = await AppBloc.requestCubit.createRequest(widget.text, 'pol', false, institutionModel?.id ?? '');
                    if (resopnse.isSuccess) {
                      Navigator.push(context, MaterialPageRoute(builder: (context) => MainCallPage(text: 'Вызвать полицию', requestId: resopnse.requestId, show: isNotifContact, type: 'pol',)));
                    }else{
                ...
                child: SvgPicture.asset(on ?'assets/images/p_on.svg' : 'assets/images/p_off.svg')),
              Text('Вызвать полицию',style: TextStyle(color: Color.fromRGBO(219, 19, 91, 1), fontSize: 18, fontWeight: FontWeight.w600),)
            ],
          ),
        ),
        SizedBox(height: 20),
        ...
        ? GestureDetector(
          onTap: ()async{
            Navigator.push(context, MaterialPageRoute(builder: (context) => SelectInstituts(type: 'pol')))
            .then((institution){
              setState(() {
